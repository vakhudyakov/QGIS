...
void QgsExtractLabelsAlgorithm::initAlgorithm( const QVariantMap & )
{
  ...
  std::unique_ptr<QgsProcessingParameterMapTheme> mapThemeParameter = std::make_unique<QgsProcessingParameterMapTheme>(
        ...
        QVariant(), true );
  mapThemeParameter->setHelp( QObject::tr( "This parameter is optional. When left unset, the algorithm will fallback to match the currently opened project's layer tree visibility to extract labels from." ) );
...
QString QgsExtractLabelsAlgorithm::shortHelpString() const
{
  return QObject::tr( "This algorithm extracts label information from a rendered map at a given extent and scale.\n\n"
                      "If a map theme is provided, the rendered map will match the visibility and symbology of that theme. If left blank, the project's layer tree setup will be used.\n\n"
...
class ExtractLabelSink : public QgsLabelSink
{
  public:
    ...
    void processLabel( const QString &layerId, QgsRenderContext &context, pal::LabelPosition *label, const QgsPalLayerSettings &settings )
    {
      ...
      double x = label->getX();
      double y = label->getY();
      if ( dataDefinedValues.contains( QgsPalLayerSettings::PositionX ) )
      {
        x = dataDefinedValues.value( QgsPalLayerSettings::PositionX ).toDouble();
      }
      if ( dataDefinedValues.contains( QgsPalLayerSettings::PositionY ) )
      {
        y = dataDefinedValues.value( QgsPalLayerSettings::PositionY ).toDouble();
      }
